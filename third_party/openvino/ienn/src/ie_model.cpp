...
// Constant with shape {1} or shape {} is a scalar.
inline bool IsScalar(ie_constant_t constant) {
  if ((constant.dimensionsCount == 1 && constant.dimensions[0] == 1) ||
      constant.dimensionsCount == 0) {
    return true;
  } else {
    return false;
  }
...
std::shared_ptr<ngraph::Node> CreateClampNode(
    ngraph::Output<ngraph::Node> input_node,
    ie_clamp_options* options) {
  std::shared_ptr<ngraph::Node> clamp_node;
  // If min value and max value are both scalars with shape {1} or shape {}, use
  ...
  // OpenVINO.
  if (IsScalar(options->min) && IsScalar(options->max)) {
    float min = options->min.value == nullptr
                    ? std::numeric_limits<float>::lowest()
                    : options->min.value[0];
    float max = options->max.value == nullptr
                    ? std::numeric_limits<float>::max()
                    : options->max.value[0];
    clamp_node = std::make_shared<op::v0::Clamp>(input_node, min, max);
  } else {
    std::shared_ptr<ngraph::Node> min_constant, max_constant;
    if (options->min.value != nullptr) {
      min_constant =
          CreateConstantNode(options->min.dimensions,
                             options->min.dimensionsCount, options->min.value);
    }

    if (options->max.value != nullptr) {
      max_constant =
          CreateConstantNode(options->max.dimensions,
                             options->max.dimensionsCount, options->max.value);
    }

    // Compare input with min value.
    auto max_node =
        options->min.value != nullptr
            ? std::make_shared<op::v1::Maximum>(input_node, min_constant)
                  ->output(0)
            : input_node;
    // Compare input with max value.
    clamp_node = options->max.value != nullptr
...
std::shared_ptr<ngraph::Node> AddActivationNode(
    ngraph::Output<ngraph::Node> input_node,
    ie_activation_t activation) {
  std::shared_ptr<ngraph::Node> activation_node;
  switch (activation.type) {
    case ie_activation_type::None:
      return input_node.get_node_shared_ptr();
      break;
    case ie_activation_type::Clamp: {
      ie_clamp_options_t clamp_options = {activation.param1, activation.param2};
      activation_node = CreateClampNode(input_node, &clamp_options);
      break;
    }
    case ie_activation_type::Relu:
      activation_node = std::make_shared<op::v0::Relu>(input_node);
      break;
    case ie_activation_type::Sigmoid:
      activation_node = std::make_shared<op::v0::Sigmoid>(input_node);
      break;
    case ie_activation_type::LeakyRelu: {
      float alpha = activation.param1.value == nullptr
                        ? 0.01
                        : activation.param1.value[0];
      const auto alpha_node =
          op::Constant::create(element::f32, Shape{1}, {alpha});
      activation_node = std::make_shared<op::v0::PRelu>(input_node, alpha_node);
      break;
    }
    default:
      assert(0);
...
ie_operand_t* Model::AddBatchNorm(ie_operand_t* input,
  ...
  //                  : batch_norm_node;
  auto transpose_node =
      nhwc ? TransposeInputLayout(batch_norm_node->output(0), false)
           : batch_norm_node;

  auto node = AddActivationNode(transpose_node->output(0), options->activation);
  std::string node_name = node->get_name();
  name_node_map_[node_name] = node->output(0);
  return CreateOperand(node_name);
}
...
ie_operand_t* Model::AddClamp(ie_operand_t* input,
  ...
  auto input_node = name_node_map_[input->name];
  std::shared_ptr<ngraph::Node> clamp_node =
      CreateClampNode(input_node, options);
...
ie_operand_t* Model::AddConv2d(ie_operand_t* input,
  ...
  auto transpose_node =
      options->inputLayout == ie_input_operand_layout::Nhwc
          ? TransposeInputLayout(conv2d_node->output(0), false)
          : conv2d_node;
  auto add_node =
      options->bias.name == nullptr
          ? transpose_node
          : std::make_shared<op::v1::Add>(transpose_node->output(0),
                                          name_node_map_[options->bias.name]);

  auto node = AddActivationNode(add_node->output(0), options->activation);
  std::string node_name = node->get_name();
  name_node_map_[node_name] = node->output(0);
  return CreateOperand(node_name);
}
...
ie_operand_t* Model::AddTanh(ie_operand_t* input) {
  auto input_node = name_node_map_[input->name];
  auto tanh_node = std::make_shared<op::v0::Tanh>(input_node);

  std::string node_name = tanh_node->get_name();
  name_node_map_[node_name] = tanh_node->output(0);
  return CreateOperand(node_name);
}
