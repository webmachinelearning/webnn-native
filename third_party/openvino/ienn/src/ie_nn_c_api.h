...
enum ie_activation_type : uint32_t {
  None = 0x00000000,
  Clamp = 0x00000001,
  Relu = 0x00000002,
  Sigmoid = 0x00000003,
  LeakyRelu = 0x00000004,
};

typedef struct ie_constant {
  const float* value = nullptr;
  const int32_t* dimensions;
  uint32_t dimensionsCount = 0;
} ie_constant_t;

typedef struct ie_activation {
  ie_activation_type type = ie_activation_type::None;
  ie_constant_t param1;
  ie_constant_t param2;
} ie_activation_t;
...
/**
 * @brief Add Tanh node to nGraph. Use the ie_operand_free() method to
 *  free the operand memory.
 * @ingroup model
 * @param ie_operand_t The input operand.
 * @return Status code of the operation: OK(0) for success.
