...
class Conv2dValidationTest : public ValidationTest {
  protected:
    void SetUp() override {
        ...
        inputDesc = {wnn::OperandType::Float32, shape.data(), (uint32_t)shape.size()};
...
TEST_F(Conv2dValidationTest, CreateByDefaultOptions) {
    // Success
    {
        ...
        wnn::Conv2dOptions conv2dOptions = {};
        wnn::Operand conv = mBuilder.Conv2d(mInput, mFilter, &conv2dOptions);
    }
    { wnn::Operand conv = mBuilder.Conv2d(mInput, mFilter); }
}
...
TEST_F(Conv2dValidationTest, DifferentTypeError) {
    ...
    wnn::OperandDescriptor inputDesc = {wnn::OperandType::Int32, shape.data(),
                                        (uint32_t)shape.size()};
    wnn::ArrayBufferView arrayBuffer = {data.data(), data.size() * sizeof(int32_t)};
    wnn::Operand filter = mBuilder.Constant(&inputDesc, &arrayBuffer);
    wnn::Conv2dOptions conv2dOptions = {};
